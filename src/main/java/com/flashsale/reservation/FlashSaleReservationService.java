 ...
 * 重复请求都不会制造无效队列流量；预扣成功但发送失败时进入补偿或对账。</p>
 */
@Service
public class FlashSaleReservationService {
    ...
    public ReservationResult reserve(long activityId, long userId, String requestId) {
        ...
        try {
            ReservationResult result = reserveInternal(activityId, userId, requestId);
            resultCode = result.code();
            return result;
        } finally {
            metrics.finishReservation(sample, resultCode);
        }
    }

    private ReservationResult reserveInternal(long activityId, long userId, String requestId) {
